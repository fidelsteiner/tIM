################################################################################
# Determine defining morphologies across Greenland ice margin
#
# IceMarginMorphologyExtractor.R
#
# ReadMe: 
#
# Uses known sector morphology distributions from selected sites to assign defining morphologies to all
# valid margin sections across Greenland
#
# https://github.com/fidelsteiner/tIM
#
# Input:
# Requires the different margin sections as polylines, slope values produced for each grid cell as well as defining
# morphologies for selected sites
#
# Created:          2021/10/15
...
folderSectors <- paste('E:\\tGISM\\Sectors',sep = "")     # path for all sector data
...
##################
# Concept Plot for Slope distributions
##################
...
##################
# Associate slope distributions from known locations with every location across Greenland to determine defining morphology
# for the respective 1 x 1 km cell (through a Kolmogorov Smirnoff Test)
##################

# Sample cells (cell ID numbers from the respective sector)
# near-vertical (shallow terrain):
# Subbasin 301: c(51963, 52256, 51962,52547, 52840, 53692, 51966)
# Subbasin 176: c(7700, 7872, 7528, 6839, 7183, 7011, 6667, 5979)

# near-vertical (steep terrain):
# Subbasin 85: c(178, 179)

# steep ramp
# Subbasin 176: c(7868, 7869, 7870, 7871, 7866)
# Subbasin 301: c(52255, 53131, 53423)

# shallow ramp
# Subbasin 176: c(9346, 9347, 9518, 9519, 9691, 9692, 9693, 9864, 9865)
# Subbasin 46: c(3053, 3128,3129, 3585, 3659, 3660)
# Subbasin 306: c(98943,98944, 99188, 99189,99191)
...
##################
# Compute defining slope morphologies for each cell (time consuming)
# main function used dts_stat()
##################
...
##################
# Compute doughnut plots with margin morphologies per region
##################
...
##################
# Combine margin types and save as regional files
##################

# GrIS
for(i in 1:7){
  
  cliffMar <- ogrInfo(paste(output_basepath,'\\Sectors\\',i,'RegionalCliffMargin.shp',sep = ""))
  cliffMar <-   readOGR(dsn=paste(output_basepath,'\\Sectors\\',i,'RegionalCliffMargin.shp',sep = ""),layer = cliffMar$layer)
  if(i==1){regionalCliffMar <- cliffMar}
  if(i>1){regionalCliffMar <- rbind(cliffMar,regionalCliffMar)}
  
  steepcliffMar <- ogrInfo(paste(output_basepath,'\\Sectors\\',i,'RegionalsteepCliffMargin.shp',sep = ""))
  steepcliffMar <-   readOGR(dsn=paste(output_basepath,'\\Sectors\\',i,'RegionalsteepCliffMargin.shp',sep = ""),layer = steepcliffMar$layer)
  if(i==1){regionalsteepCliffMar <- steepcliffMar}
  if(i>1){regionalsteepCliffMar <- rbind(steepcliffMar,regionalsteepCliffMar)}
  
  steepMar <- ogrInfo(paste(output_basepath,'\\Sectors\\',i,'RegionalSteepMargin.shp',sep = ""))
  steepMar <-   readOGR(dsn=paste(output_basepath,'\\Sectors\\',i,'RegionalSteepMargin.shp',sep = ""),layer = steepMar$layer)
  if(i==1){regionalSteepMar <- steepMar}
  if(i>1){regionalSteepMar <- rbind(steepMar,regionalSteepMar)}
 
  shallowMar <- ogrInfo(paste(output_basepath,'\\Sectors\\',i,'RegionalShallowMargin.shp',sep = ""))
  shallowMar <-   readOGR(dsn=paste(output_basepath,'\\Sectors\\',i,'RegionalShallowMargin.shp',sep = ""),layer = shallowMar$layer)
  if(i==1){regionalShallowMar <- shallowMar}
  if(i>1){regionalShallowMar <- rbind(shallowMar,regionalShallowMar)}
  
}
rgdal::writeOGR(regionalCliffMar,paste(output_basepath,'\\Sectors\\RegionalCliffMargin_GrIS.shp',sep = ""),layer = 'regionalCliffMargin',driver = 'ESRI Shapefile',overwrite_layer = T)
rgdal::writeOGR(regionalsteepCliffMar,paste(output_basepath,'\\Sectors\\RegionalsteepCliffMargin_GrIS.shp',sep = ""),layer = 'regionalsteepCliffMargin',driver = 'ESRI Shapefile',overwrite_layer = T)
rgdal::writeOGR(regionalSteepMar,paste(output_basepath,'\\Sectors\\RegionalSteepMargin_GrIS.shp',sep = ""),layer = 'regionalSteepMargin',driver = 'ESRI Shapefile',overwrite_layer = T)
rgdal::writeOGR(regionalShallowMar,paste(output_basepath,'\\Sectors\\RegionalShallowMargin_GrIS.shp',sep = ""),layer = 'regionalShallowMargin',driver = 'ESRI Shapefile',overwrite_layer = T)
...
rgdal::writeOGR(regionalCliffMar,paste(output_basepath,'\\Sectors\\RegionalCliffMargin_PGIC.shp',sep = ""),layer = 'regionalCliffMargin',driver = 'ESRI Shapefile',overwrite_layer = T)
rgdal::writeOGR(regionalsteepCliffMar,paste(output_basepath,'\\Sectors\\RegionalsteepCliffMargin_PGIC.shp',sep = ""),layer = 'regionalsteepCliffMargin',driver = 'ESRI Shapefile',overwrite_layer = T)
rgdal::writeOGR(regionalSteepMar,paste(output_basepath,'\\Sectors\\RegionalSteepMargin_PGIC.shp',sep = ""),layer = 'regionalSteepMargin',driver = 'ESRI Shapefile',overwrite_layer = T)
rgdal::writeOGR(regionalShallowMar,paste(output_basepath,'\\Sectors\\RegionalShallowMargin_PGIC.shp',sep = ""),layer = 'regionalShallowMargin',driver = 'ESRI Shapefile',overwrite_layer = T)
