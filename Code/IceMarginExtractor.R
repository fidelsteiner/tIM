...
#
# ReadMe: Primary code to extract morphologies from the ArcticDEM and available ice masks
# Note that due to the large amount of data, data types, inclusion of manually checked sections, reprojections
# and general size of data (ArcticDEM tiles, potentially also updated in future) this code is not plug and play. Adjustments for local deployment
# need to be carefully made depending on the location of data on the local machine
# Processing can take multiple hours for all of Greenland on a single core
#
# https://github.com/fidelsteiner/tIM
#
# Input:
# (1) ice masks of GrIS and PGIC (.shp, currently based on Rastner et al. 2012, see manuscript)
# (2) ArcticDEM tiles (.tif, not provided in manuscript, need to be obtained from https://arcticdem.apps.pgc.umn.edu/)
# (3) Bedmachine topography data (.tif)
# (4) Lake inventory (.shp)
#
# Created:          2021/10/15
...
path_figs <- paste(output_basepath,'\\Figures',sep = "")  # path for output figures
...
# combination of GrIS and PGIC
margTotal <- rbind(icemargin_total,icemargin_total2)

# Load seperate sector segments
...
# Prepare slope from base DEMs (skipped if slope maps already downloaded)
...
# Core Processing
################################
# loop through each sector across Greenland processing the mask, buffer and extracting slope values for each 1km cell
...
if(!file.exists(paste(output_basepath2,'\\Sectors\\',SubBasinID,'Sector\\',SubBasinID,'progress_1k.txt',sep = ""))){
  ...
  ################################ 
  # (1) Read in basic data and prepare lines for each sector
  ################################
...
# Open statistics file for margin lengths
...
################################ 
# (2) Separate terrestrial from marine-terminating margin
################################

# analyse the margin properties with respect to the bed topography (to differentiate between )
...
# Limit extents for individual subbasins where processing tends to crash due to large extent (can be potentially removed)
if(SubBasinID==260){
...
################################ 
# (3) Separate land-terminating from lake-terminating margin
################################
...
################################ 
# (4) Process segments for storage and to calculate statistics
################################
...
# read in margins of manually corrected subbasins (see Supplementary Material for documentation)
...
################################ 
# (5) Processing of morphology through cells along margin segments
################################
...
################################ 
# (5.1) Create grid of 1km cells and store shp file
################################
...
################################ 
# (5.2) loop through all DEM outlines to see which ones overlap
################################
...
################################ 
# (5.3) Extract morphology/slope data
################################
...
################################ 
# (5.4) Save output for this subbasin
################################
